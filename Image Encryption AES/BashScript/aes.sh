...
while getopts "d:e:sphk:" opt; do
    case $opt in
        ...
        h) echo "Usage: $0 [-d input_directory] [-e executable_path] [-s (sequential)] [-p (parallel)] [-k key_type (128/192/256)]" ; exit 0 ;;
        *) echo "Usage: $0 [-d input_directory] [-e executable_path] [-s (sequential)] [-p (parallel)] [-k key_type (128/192/256)]" ; exit 1 ;;
    esac
done
